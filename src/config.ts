 ...
 *  1. MYSQL_MCP_CONFIG env var pointing to a JSON file
 *  2. MYSQL_MCP_CONFIG_JSON env var containing inline JSON
 *  3. Individual env vars for a single connection (MYSQL_HOST, etc.)
 */
export function loadConfig(): AppConfig {
  // Option 1: Config file path
  const configPath = process.env.MYSQL_MCP_CONFIG;
  if (configPath) {
  ...
  const configJson = process.env.MYSQL_MCP_CONFIG_JSON;
  if (configJson) {
  ...
  if (!host) {
    throw new Error(
      "No configuration found. Set MYSQL_MCP_CONFIG (file path), " +
        "MYSQL_MCP_CONFIG_JSON (inline JSON), or MYSQL_HOST + related env vars."
    );
  }
...
function parseConfig(raw: unknown): AppConfig {
  ...
  const connections: DatabaseConfig[] = arr.map(
